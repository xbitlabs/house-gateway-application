package com.nezha.house.web.rest;

import com.nezha.house.config.KafkaProperties;
...
class HouseGatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        HouseGatewayKafkaResource kafkaResource = new HouseGatewayKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/house-gateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/house-gateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
